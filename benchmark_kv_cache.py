...
def autoregressive_generate(model, inputs, max_new_tokens, use_cache=False):
    ...
    return (end - start) * 1000, mem_used


def custom_kv_cache_benchmark(model, inputs, max_new_tokens):
...
print("Running benchmark with Custom KV-cache...")
torch.cuda.empty_cache()
time_custom, mem_custom = custom_kv_cache_benchmark(model, inputs, max_new_tokens)
print(f"\nBenchmark Summary")
